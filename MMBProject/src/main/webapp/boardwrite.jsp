...
<script>
	...
	function findDeletedImages(previousData, currentData) {
     ...
     // (/<img[^>]+src="([^">]+)"/g)
     // <img
     // img 태그의 시작 부분을 찾습니다. <img로 시작하는 태그를 찾습니다.
     
     // [^>]+ 
     // [^>]는 '>'가 아닌 모든 문자에 일치합니다. +는 하나 이상의 연속된 문자에 일치합니다. 따라서, [^>]+는 '>'가 아닌 모든 문자가 하나 이상 연속된 것을 의미합니다.
       // 이는 <img 태그의 끝까지 또는 src 속성까지 포함할 수 있는 모든 속성을 포함합니다.   
       
       // src="
       // src="는 이미지 태그에서 src 속성이 시작하는 부분을 찾습니다.
       // src 속성의 값은 "로 시작하고, 그 뒤에 URL이 옵니다.
       
       // ([^">]+)
     // [^">]는 '">'가 아닌 모든 문자에 일치합니다.
     // +는 하나 이상의 연속된 문자에 일치합니다.
     // 따라서, ([^">]+)는 '">'가 아닌 모든 문자가 하나 이상 연속된 부분을 캡처합니다.
     // 즉, src 속성의 값인 이미지 URL을 캡처하는 부분입니다.
     // 캡처 그룹은 ()로 둘러싸여 있으며, 이를 통해 나중에 이 부분만을 추출할 수 있습니다.
     ...
     // /g
     // g 플래그는 'global'을 의미하며, 전체 문자열에서 모든 일치를 찾도록 합니다.
     // 이 플래그가 없으면 첫 번째 일치만 반환됩니다.
     
     // 요약
     // <img                // <img로 시작하는 태그를 찾습니다.
     // [^>]+               // '>'가 아닌 모든 문자가 하나 이상 연속된 것을 포함합니다. (속성들 포함)
     // src="               // src="로 시작하는 부분을 찾습니다. (src 속성의 시작)
     // ([^">]+)            // '">'가 아닌 모든 문자를 캡처합니다. (이미지 URL)
     // "                   // URL의 끝을 나타내는 " 문자
     //  /g                 // 전체 문자열에서 모든 일치를 찾습니다. (global)
